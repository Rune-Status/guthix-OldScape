    ...
    companion object {
        ...
        fun decode(buffer: ByteBuffer): AudioInstrument {
            ...
            val (pitchModifier, pitchModifierAmplitude) = if (buffer.uPeak().toInt() != 0) {
                Pair(AudioEnvelope.decode(buffer), AudioEnvelope.decode(buffer))
            } else Pair(null, null)
            ...
            return AudioInstrument(pitch, volume, pitchModifier, pitchModifierAmplitude, volumeMultiplier,
...
class AudioFilter @ExperimentalUnsignedTypes constructor(
    ...
    companion object {
        ...
        fun decode(buffer: ByteBuffer, audioEnvelope: AudioEnvelope): AudioFilter {
            ...
            val phases = Array(SIZE) { Array(SIZE) { UShortArray(SIZE * 2) } }
            val magnitudes = Array(SIZE) { Array(SIZE) { UShortArray(SIZE * 2) } }
